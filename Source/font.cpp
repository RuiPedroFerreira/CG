...
GLuint initFont() {
    ...
    const GLchar* vertexSource = load_shader_source(std::string("..\\..\\Shaders\\fontVertex.shader"))->c_str();
    compile_shader(vertexShader, vertexSource);

    // FRAGMENT SHADER
    GLuint fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    const GLchar* fragmentSource = load_shader_source(std::string("..\\..\\Shaders\\fontFragment.shader"))->c_str();
    ...
    FT_Face face;
    if (FT_New_Face(ft, "fonts/bauh/bauhs93.ttf", 0, &face)) {
    ...
    glm::mat4 projection = glm::ortho(0.0f, 512.0f, 0.0f, 512.0f);
